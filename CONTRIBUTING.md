...
- `LIA_TEST_ONLY="lease,bundle"` runs only the tests whose name contains one
  of those words (quick iteration; a release always runs the whole suite).
- Tests use fictional names, addresses and meeting content only. The
  publication-guard test fails on known personal data; add your own private
  terms with `LIA_PRIVATE_DENYLIST=<a file outside the repo, one per line>`.

Expected: everything passes except the live-API tests, which are skipped.
