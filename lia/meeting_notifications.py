...
class MeetingCard:
    ...
    def __init__(self, root, tk_queue, dpi_scale=1.0, logo_image=None,
        ...
        # The user's Minimize / Details choice for the CURRENT card (same
        # `anchor`). The host re-renders on every stage change from its own
        # model, which does not know the choice: without this the card popped
        # back open on the next update (2026-09-23: "it won't minimize").
        self._ui = {}
    ...
    def render(self, state):
        ...
        st = dict(state or {})
        prev = self._state or {}
        if st.get("anchor") is not None and st.get("anchor") == prev.get("anchor"):
            st.update(self._ui)           # same card: keep the user's choice
        else:
            self._ui = {}                 # a new card starts open
        self._build(st)
    ...
    def _build(self, c):
        ...
        m = self._m
        m.rtl = self._rtl            # Hebrew needs a face that covers it
        pad = m.px(ns.PAD)
        ...
        if c.get("minimized") and c.get("allow_min"):
            # Minimized: one compact line (state + elapsed) under the brand row;
            # "+" restores. Name, stages and actions come back on restore.
            self._build_compact(tk, body, c)
        else:
    ...
    def _build_brand(self, tk, parent, c):
        ...
        if c.get("allow_min"):
            self._widgets["min_btn"] = self._icon_btn(
                tk, row, "+" if c.get("minimized") else "–", self._on_min,
    ...
    def _build_compact(self, tk, parent, c):
        """The minimized card: spinner + title + elapsed on one small line."""
        m = self._m
        row = tk.Frame(parent, bg=ns.BG)
        row.pack(fill="x", pady=(m.px(4), 0))
        lead = "right" if self._rtl else "left"
        trail = "left" if self._rtl else "right"
        if c.get("dot") == "busy":
            self._widgets["spin"] = tk.Label(
                row, text=_SPIN[0], bg=ns.BG, fg=_dot_color("busy"),
                font=m.font(ns.PT_META))
            self._widgets["spin"].pack(side=lead, padx=(0, m.px(6)))
        tk.Label(row, text=c.get("title", ""), bg=ns.BG, fg=ns.INK,
                 font=m.font(ns.PT_META, semibold=True)).pack(side=lead)
    ...
    def _build_info(self, tk, parent, c):
        ...
        if chips and c.get("dot") == "busy":
            # Stage + elapsed on their OWN line: next to a long engine chip the
            # stage text was cut off ("scribing · 127/756", 2026-09-23).
            row = tk.Frame(parent, bg=ns.BG)
            row.pack(fill="x", pady=(m.px(4), 0))
        if c.get("dot") == "busy":
    ...
    # ---- button handlers -------------------------------------------------
    # Handlers run on a worker thread (_fire); every Tk change hops back to the
    # Tk thread via post - Tk widgets must not be built off their own thread.
    def _on_close(self):
        pol = (self._state or {}).get("on_close")
        if pol:
            self._safe_cb(pol)       # e.g. the stop-prompt's "keep recording"
        self.post(self._teardown)

    def _toggle_ui(self, key):
        def _do():
            c = self._state
            if not c:
                return
    ...
    def _on_min(self):
        self._toggle_ui("minimized")

    def _on_toggle_details(self):
        self._toggle_ui("expanded")
