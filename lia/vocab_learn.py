...
# C0 / DEL / C1 control characters (incl. newline, CR, tab): never part of a
# correction - a newline in `right` would split a pasted dictation into lines
# (a terminal or chat with press-Enter submits each one). Audit 2026-09-26 F3.
_CONTROL_RE = re.compile(r"[\x00-\x1f\x7f-\x9f]")


def correction_pair_problem(wrong, right, strict=True):
    """Why a {wrong, right} pair must not be stored (text), or None. strict =
    a pair that came from a model / a meeting (not typed by the user): no URL
    scheme and no backslash on the right side either."""
    if not wrong or not right:
        return "empty"
    if _CONTROL_RE.search(wrong) or _CONTROL_RE.search(right):
        return "control characters"
    if not (2 <= len(wrong) <= 40) or not (2 <= len(right) <= 40):
        return "length"
    if strict and ("://" in right or "\\" in right):
        return "a URL or a backslash"
    return None
...
def apply_corrections(text, pairs):
    ...
    `right` is inserted LITERALLY (it used to be a regex template: "\\n" became a
    newline and a bad escape raised, which silently disabled every correction),
    and one broken pair never stops the others."""
    counts = {}
    ...
    for p in pairs:
        try:
            ...
            if not wrong or not right or _norm(wrong) == _norm(right):
                continue
            if _CONTROL_RE.search(right):
                continue
            text, n = _correction_re(wrong).subn(lambda _m, r=right: r, text)
            if n:
...
class VocabStore:
    ...
    def add_corrections(self, pairs, source="llm", force=False):
        ...
        with self._lock:
            for p in pairs or []:
                wrong = (p.get("wrong") or "").strip()
                right = (p.get("right") or "").strip()
                if correction_pair_problem(wrong, right, strict=not force):
                    continue
                wk = _norm(wrong)
                if wk == _norm(right):
                    continue
    ...
    def add_correction_suggestions(self, pairs, label="summary"):
        """A PROPOSED wrong->right fix (the summary model's side-task) becomes a
        suggestion with its proposal pre-filled - NOT an auto-applied rule. As
        global rules they rewrote real words everywhere ('למדה' -> 'Lambda' was
        applied 8x, 2026-09-23). Same guards as add_corrections; a pair whose
        `wrong` is already corrected or an approved term is skipped. Returns
        #added/updated."""
        today = time.strftime("%Y-%m-%d")
        added = 0
        with self._lock:
            for p in pairs or []:
                wrong = (p.get("wrong") or "").strip()
                right = (p.get("right") or "").strip()
                if correction_pair_problem(wrong, right, strict=True):
                    continue
                wk = _norm(wrong)
                if wk == _norm(right):
                    continue
                ...
                o = self._oov.get(wk)
                if o is None:
                    self._oov[wk] = {"word": wrong, "count": 1, "first_seen": today,
                                     "last_seen": today, "label": label,
                                     "proposed": right}
                else:
                    o["count"] = int(o.get("count", 0)) + 1
    ...
    def demote_corrections(self, source, label="summary"):
        """Move every correction from `source` back to a suggestion (proposal
        kept, one click re-adds it). Returns #moved."""
        moved = []
        with self._lock:
            for wk, c in list(self._corrections.items()):
                if c.get("source") == source:
                    moved.append({"wrong": c.get("wrong"), "right": c.get("right")})
                    del self._corrections[wk]
        if moved:
            self.save()
