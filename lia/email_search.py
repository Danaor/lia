...
class SearchApi:
    ...
    def prewarm(self, model=None):
        ...
        is_local = ei.is_local_url(self._base_url)
        ...
        def _w():
            # Under the GPU gate + lease (2026-09-24): only when the GPU is free
            # now - never into a card without room, never behind another app's
            # job (the first real question then waits for it instead). The
            # ask's window, or the first ask reloads (P2.2).
            import ollama_gate
            ollama_gate.prewarm(model, self._base_url, ei.ASK_NUM_CTX)
...
/* "GPU busy - ..." while a local answer waits for the GPU (ollama_gate) */
window.__gpuWait=function(t){
  var h=document.querySelector('#preview .ans-head');
  if(h && h.querySelector('.spin')){ h.innerHTML='<span class="spin"></span> '+esc(t); }
};
...
def main():
    ...
    try:
        win = webview.create_window(payload.get("title", "Email search"),
                                    html=_with_csp(HTML), js_api=api,
                                    width=1180, height=780, min_size=(820, 560))
        import ollama_gate
        ollama_gate.UI["on_wait"] = lambda t: win.evaluate_js(
            "window.__gpuWait && window.__gpuWait(%s)" % json.dumps(t))
