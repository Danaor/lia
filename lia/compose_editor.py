...
try:
    from lia import (OpenAILLMCleaner, load_config, SUMMARIES_DIR,
                             _txt_apply_rtl_to_doc, _compose_model)
except Exception as e:  # pragma: no cover
...
def _safe_shell(s, head=False):
    """The file shell around the summary card with active content removed:
    <script>/<iframe>/<object>/<embed>/<link>/<base>/<form> elements, meta
    refresh, on*= handlers, javascript: URLs, remote url()/@import in <style>.
    head=True also adds the no-network CSP for the saved document."""
    import re as _re
    s = _re.sub(r"(?is)<(script|iframe|object|embed|form|svg|math)\b.*?</\1\s*>", "", s)
    s = _re.sub(r"(?is)<(script|iframe|object|embed|link|base|form|input|svg|math"
                r"|img|audio|video|source|track|frame|frameset)\b[^>]*>", "", s)
    s = _re.sub(r"(?is)<meta[^>]*http-equiv\s*=\s*['\"]?refresh[^>]*>", "", s)
    s = _re.sub(r"(?is)\s+on[a-z]+\s*=\s*(\"[^\"]*\"|'[^']*'|[^\s>]+)", "", s)
    s = _re.sub(r"(?is)javascript\s*:", "blocked:", s)
    s = _re.sub(r"(?is)@import[^;]*;?", "", s)
    s = _re.sub(r"(?is)url\s*\([^)]*\)", "none", s)
    if head and "Content-Security-Policy" not in s:
        s = _re.sub(r"(?i)<head>", "<head>\n" + _SAVED_DOC_CSP, s, count=1)
    return s


# A saved / shared summary never needs script or the network (audit F13).
_SAVED_DOC_CSP = ('<meta http-equiv="Content-Security-Policy" content="default-src '
...
class SummaryApi:
    ...
    def _parse(self):
        ...
        # The file's own CSS only when it cannot fetch anything or run code
        # (audit F13: an @import / url() beaconed out from this privileged
        # window); otherwise Lia's own summary CSS.
        if m and m.group(1).strip() and not _re.search(
        ...
        # Saved back around the edited card: never with a script, a remote
        # resource or an event handler the original file carried (audit F13 -
        # Save / Email would otherwise ship it onward), and with a CSP.
        self._prefix = _safe_shell(o[:open_end], head=True)
        self._suffix = _safe_shell(o[k:])
...
def _run_summary_editor(payload):
    ...
    try:
        win = webview.create_window(payload.get("title", "Edit summary"),
                                    html=_with_csp(SUMMARY_HTML), js_api=api,
...
def main():
    ...
    if key and OpenAILLMCleaner is not None:
        try:
            # The Compose model, like the parent's _get_compose_cleaner. It read
            # summary_model, so a gemma / Gemini / "off" summary choice sent that
            # name to OpenAI and Revise / Regenerate failed.
            cleaner = OpenAILLMCleaner(api_key=key, model=_compose_model(cfg))
        except Exception as e:
            sys.stderr.write("compose_editor: cleaner init failed: %r\n" % (e,))

    api = Api(payload, cleaner)
    win = webview.create_window(payload.get("title", "Compose"), html=_with_csp(HTML),
