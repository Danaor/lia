...
TK_GRAFT_FILES = ("_tkinter.pyd", "tcl86t.dll", "tk86t.dll", "zlib1.dll")
...
def phase_3_graft_tkinter():
    ...
    # Only what tkinter needs (audit 2026-09-26 F27): the embeddable already
    # ships the rest, and copying every DLL from the BUILD machine's Python
    # would carry whatever else sits there into the release.
    for item in TK_GRAFT_FILES:
        src = os.path.join(dlls_src, item)
        if not os.path.isfile(src):
            raise RuntimeError("tkinter graft: %s missing in %s" % (item, dlls_src))
...
def phase_5_create_launcher():
    ...
    try:
        ...
        v = (1, 6, 11, 0)   # app version (was derived from the Python version by mistake)
        vi = VSVersionInfo(
...
# Developer tools that must not ship inside a release's app\ folder (audit
# 2026-09-26 D6): the test suite (it runs in phase 8 first) and the benches,
# galleries and build scripts. They stay in the public repository.
DEV_ONLY_FILES = ("run_tests.py", "bench_qmark.py", "bench_speakers.py",
                  "ui_kit_gallery.py", "notification_preview.py", "build.py",
                  "build_runtime.py", "build_installer.py", "make_checksums.py",
                  "generate_logo.py", "installer.iss")


def phase_8b_strip_dev_files():
    print("\n=== Phase 8b: strip developer-only files from app\\ ===")
    n = 0
    for name in DEV_ONLY_FILES:
        ...
        if os.path.exists(p):
            os.remove(p)
            n += 1
    print(f"  Removed {n} developer-only file(s)")
...
def main():
    ...
    if os.path.isdir(RUNTIME_DIR) and os.listdir(RUNTIME_DIR):
        # A build without --clean would keep an OLD app\ (sources that may not
        # match HEAD) - never a release (audit 2026-09-26 F27).
        print(f"Build dir exists ({BUILD_DIR}). Rebuild with --clean.")
        sys.exit(1)
