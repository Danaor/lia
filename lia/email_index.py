...
_LOCAL_URL_RE = re.compile(r"^https?://(localhost|127\.0\.0\.1|\[::1\])(:\d+)?(/|$)", re.I)


def is_local_url(url):
    """True only for a loopback http(s) URL (strict: 'http://evil/?127.0.0.1'
    or 'http://localhost.evil.com' are NOT local)."""
    return bool(_LOCAL_URL_RE.match((url or "").strip()))


def _ollama_root(base_url):
    """The local Ollama server root. Refuses anything that is not loopback
    (2026-09-26 audit F2: a cloud summary URL once reached here and e-mail
    chunks were posted to Google)."""
    h = (base_url or DEFAULT_OLLAMA).rstrip("/")
    h = re.sub(r"/v1/chat/completions$", "", h)
    h = re.sub(r"/v1$", "", h)
    if not is_local_url(h + "/"):
        raise ValueError("Ollama must be a local server (got a non-local address)")
    return h


def _is_reasoning_model(model):
    """gpt-5 and every later major (gpt-6-*) + the o-series: these reject
    `max_tokens` and a custom temperature. Keep in sync with
    lia._is_reasoning_model."""
    m = (model or "").lower()
    if m.startswith(("o1", "o3", "o4")):
        return True
    g = re.match(r"gpt-(\d+)", m)
    return bool(g) and int(g.group(1)) >= 5
...
def _llm_chat(system_prompt, user_content, base_url, model, api_key="ollama",
    ...
    if not is_local and (not api_key or api_key == "ollama"):
        # A "local" provider pointed at a non-local URL: never send the prompt.
        raise ValueError("the local model's address is not local - refused")
    sess = requests.Session()
    if is_local:
        import ollama_gate
        url = _ollama_root(base_url) + "/api/chat"
        # ONE fixed window for a fresh load (P2.2): Ollama reloads the model
        # (~7.5 s) whenever num_ctx changes, so the old exact size (len/1.9+4096)
        # made nearly every question a reload AND wasted the prewarm. A 20-hit
        # ask needs ~20-28K anyway; the cap costs ~0.3 GB more. The call runs
        # under the GPU gate + lease (ollama_gate, 2026-09-24): a runner already
        # loaded whole with room for THIS question is reused at its own window
        # (the other app's too - no reload under it).
        need = ollama_gate.ctx_for(len(system_prompt or "") + len(user_content or ""),
                                   ASK_ANSWER_RESERVE)
        with ollama_gate.local_call(model, base_url, need, ASK_NUM_CTX, job="ask",
                                    detail="Ask (meetings / email)") as num_ctx:
...
# The ask's num_ctx AND the prewarm's (email_search.prewarm): equal, or the
# prewarm loads a window the first real ask immediately reloads (P2.2).
ASK_NUM_CTX = _SUMMARY_CTX_CAP
# Tokens kept free for the answer when judging whether a resident runner's
# context is big enough for a question (2048 tokens = ~3,900 Hebrew chars).
ASK_ANSWER_RESERVE = 2048
