...
) else if exist "%~dp0..\runtime\Lia.exe" (
    start "" "%~dp0..\runtime\Lia.exe" "%~dp0lia.py"
) else (
    rem A raw source checkout: "start" searches the CURRENT folder before PATH,
    rem so run the bare name from System32 (audit 2026-09-26 F20 / old #7).
