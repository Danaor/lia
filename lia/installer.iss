...
Name: "elevatedautostart"; Description: "{cm:AutoStartElevated}"; GroupDescription: "{cm:AutoStartTaskGroup}"; Flags: unchecked; Check: IsAdminInstallMode and IsProtectedAppDir
...
; Relaunch after a SILENT in-app update (Settings / tray "Update Lia" runs
; Setup /SILENT, which skips the entry above). As the original user, so an
; elevated install never starts Lia elevated.
Filename: "{app}\runtime\{#AppExeName}"; \
    Parameters: """{app}\app\lia.py"""; \
    WorkingDir: "{app}\app"; \
...
[Code]
// Audit 2026-09-26 F6: the ELEVATED logon task may only point at a folder
// under Program Files (only admins can change it). A custom folder such as
// C:\Lia is writable by every user -> the task is not offered / not created.
function IsProtectedAppDir(): Boolean;
var
  App, Pf, Pf32: string;
begin
  App := Lowercase(AddBackslash(ExpandConstant('{app}')));
  Pf := Lowercase(AddBackslash(ExpandConstant('{commonpf}')));
  Pf32 := Lowercase(AddBackslash(ExpandConstant('{commonpf32}')));
  Result := (Pos(Pf, App) = 1) or (Pos(Pf32, App) = 1);
end;

// Audit F19: relaunch after a silent install ONLY for Lia's own in-app update
// (it passes /LIAUPDATE=1), never for an admin's silent deployment - which may
// run as SYSTEM.
function IsLiaUpdate(): Boolean;
begin
  Result := (ExpandConstant('{param:LIAUPDATE|0}') = '1') and
            (CompareText(GetUserNameString(), 'SYSTEM') <> 0);
end;
