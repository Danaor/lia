...
Setup (one-time, FREE):
  1) pip install pyannote.audio==4.0.7 torch==2.11.0 torchaudio==2.11.0
     (the tested versions; the community-1 model needs pyannote 4.x)
...
# Reviewed pipeline revisions (audit 2026-09-26 F21): the checkpoints are
# PyTorch pickles, so a moved upstream branch must never be loaded unseen.
PIPELINE_REVISIONS = {
        ...
        try:
            try:
                pipe = Pipeline.from_pretrained(m, token=auth,         # 4.x
                                                revision=PIPELINE_REVISIONS.get(m))
...
def main():
    ...
    try:
        ...
        _emit({"ok": False,
               ...
               "hint": "pip install pyannote.audio==4.0.7 torch==2.11.0 torchaudio==2.11.0"})
        return
