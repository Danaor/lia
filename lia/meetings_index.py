...
def main():
    ...
    if sys.stdout is None or sys.stderr is None:
        try:
            ensure_dir()
            _lp = os.path.join(MEET_INDEX_DIR, "indexer.log")
            try:              # a size cap: one rotation at 2 MB (audit F17)
                if os.path.getsize(_lp) > 2 * 1024 * 1024:
                    os.replace(_lp, _lp + ".1")
            except OSError:
                pass
            _sink = open(_lp, "a", encoding="utf-8")
        except Exception:
