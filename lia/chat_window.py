...
(Gemma 4) by default, or cloud gpt-6-sol if an OpenAI key is set.
...
class ChatApi:
    ...
    def _run(self, gen, messages, model):
        ...
        except ollama_gate.GpuWaitCancelled:
            self._emit(gen, "err", "Stopped while waiting for the GPU")
        except Exception as e:
    ...
    def _stream_local(self, gen, messages, model):
        ...
        # Bucket to 4096 (P2.2): Ollama reloads the model (~7.5 s) whenever
        # num_ctx changes; an exact size made every chat turn a reload.
        need = ollama_gate.ctx_for(chars, 4096)
        push, flush = self._flusher(gen)
        full = []
        hint = (" - press Stop and pick the cloud model to answer now"
                if any(m[1] in self._cloud for m in self._models) else "")
        # The GPU gate + lease (2026-09-24): the whole streamed answer holds
        # it; a runner already loaded whole with room for this turn is reused
        # at its own window. The wait shows in the answer bubble; Stop cancels.
        with ollama_gate.local_call(
                model, self._base_url, need, job="chat", detail="Chat window",
                on_wait=lambda t: self._emit(gen, "wait", t + hint),
                should_cancel=lambda: self._stop or gen != self._gen) as num_ctx:
            body = {
    ...
    def _stream_cloud(self, gen, messages, model):
        ...
        if ei._is_reasoning_model(model):
...
window.__chat = function(kind, gen, a){
  if(kind==='tok'){
    ...
    // "GPU busy - ..." until the first token (a local model waiting for the GPU)
    if(curBot && !curText){ curBot.innerHTML = '<span style="color:#8a5a00">'+esc(a)+'</span><span class="cursor"></span>'; }
  } else if(kind==='done'){
    finishBot(a || curText);
  } else if(kind==='err'){
    onErr(a);
...
def main():
    ...
    try:
        _WINDOW = webview.create_window(
            payload.get("title", "Chat"), html=_with_csp(HTML), js_api=api,
