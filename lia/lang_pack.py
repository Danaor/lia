...
CLOUD_PARITY_ADDENDUM_EN = (
    ...
    # Coverage rules (2026-09-25) - the mirror of the he block (measured in he).
    "- Coverage: every topic the participants actually discussed appears at least once, "
    "even without a decision - a client requirement or request, a scope item, an "
    "expectation of whoever does the work, the working and engagement model, a separation "
    "between organisations, environments or accounts. Such a topic with no task goes into "
    "Key Highlights; when there is no room, merge related topics into one bullet. The 3-6 "
    "limit is never a reason to drop a topic.\n"
    "- In a kickoff or a first meeting with a client, the client's requirements and "
    "expectations and the working model are the core of the summary, not only the "
    "decisions.\n"
    "- Small talk, stories and examples from other projects are not tasks."
)
